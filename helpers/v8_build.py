...
def install_depot_tools():
    if not os.path.isdir(local_path("../py_mini_racer/extension/depot_tools")):
        ...
        call(
            f'git clone https://chromium.googlesource.com/chromium/tools/depot_tools.git {local_path("../py_mini_racer/extension/depot_tools")}'
        )
...
def gen_makefiles(build_path, no_sysroot=False):
    with chdir(local_path("../py_mini_racer/extension")):
        ...
        with open(os.path.join(build_path, "args.gn"), "w") as f:
            ...
            f.write("\n".join(f"{a}={b}" for (a, b) in opts.items()))
            f.write("\n")
            if extra_args := os.getenv("GN_ARGS"):
                f.write("\n".join(extra_args.split()))
                f.write("\n")
        call(f"gn gen {local_path(build_path)}")
...
def make(build_path, target, cmd_prefix=""):
    ...
    """
    with chdir(local_path("../py_mini_racer/extension")):
        call(f"{cmd_prefix} ninja -vv -C {local_path(build_path)} {target}")
